...
样品-X 是语音工作台 X-010 档案使用的**可选**本地准实时识别引擎。主后端会按需启动本目录中的独立服务；未准备模型资产时，其余档案不受影响。
...
两个脚本分别创建 `.venv/` 与 `.venv-cuda/`，互不影响。启动时优先使用 `.venv-cuda`，校验通过后才启用，否则回退 `.venv`；也可用环境变量 `SAMPLEX_PYTHON` 指定解释器。

## 行为与限制

- 浏览器 `getUserMedia → AudioContext(16k) → AudioWorklet` 采集，每 100ms 发送 PCM16；电脑音频使用浏览器共享，WAV 按原时长重放。
