...
try {
  if(engine==='webkit') {
    ...
    }
    {
      ...
      assert.equal(await page.locator('#stage').evaluate(el=>el.inert),true);
      await page.locator('.entry-start').press('Enter');await waitStart(page);
      assert.equal(await page.evaluate(()=>window.rhine.stats().audio.state),'locked');
      assert.equal(await page.evaluate(()=>window.rhine.stats().mode),'archive');
      report.checks.push({name:'Existing silent preference still waits for entry, then enters without opening audio'});await context.close();
    }
    {
