...
const saved = new Set<string>(readLocal<string[]>("rhine-speech-saved", [])
...
const functionTitle = createRollingText($("#selected-function"), { ...textOptions, text: records[selected].title });
const rollingTitles = [functionTitle, selectionTitle, columnTitle, hoverTitle, categoryTitle, clearanceTitle];
...
let speechCapturing = false;
let sampleXCapturing = false;
function configureAudio() { audio.configure({ ...prefs, sound: prefs.sound && !speechCapturing && !sampleXCapturing, music: prefs.music && !musicSuppressed && !speechCapturing && !sampleXCapturing }); }
configureAudio();
...
// The ready screen is part of the normal entrance, even with saved mute prefs.
// Only wallpaper hosting and explicit frame/scene reviews bypass the gesture.
const entry = !isWallpaper && !reviewEntry ? new StartupGate({
  root: loading,
...
function updateSelection(navigation?: ArchiveNavigation) {
  ...
  functionTitle.update({ text: r.title, animated: !prefs.reduced && mode === "archive" });
  selectionTitle.update({ text: r.id + " / " + r.category, animated: !prefs.reduced && mode === "archive" });
  ...
  fileTicks.forEach((button, slot) => {
    ...
    if (!record) { button.removeAttribute('data-select'); return; }
...
function toggleSaved() {
  ...
  try {
    localStorage.setItem("rhine-speech-saved", JSON.stringify([...saved]));
  } catch {}
  $("#saved-count").textContent = String(saved.size).padStart(2, "0");
...
function renderDetail() {
  ...
  $("#detail-content").innerHTML = `
    ...
    <h2>${escapeHtml(r.en)}</h2>
    <div class="detail-title-cn">${escapeHtml(r.title)}</div>
    <div class="detail-rule"></div>
    <p class="speech-archive-summary">${escapeHtml(r.abstract)}</p>
...
document.addEventListener("keydown", (e) => {
  ...
  if (typing || modal || !ready || (e.target as HTMLElement).closest('#speech-archive')) return;
  if (
