...
export type SpeechFeature = 'live' | 'record' | 'target' | 'meeting' | 'voice' | 'models';
const entries: { id: string; title: string; en: string; category: string; feature: SpeechFeature; model: string; abstract: string; findings: string[] }[] = [
  { id: 'X-001', title: '非实时识别 · SenseVoice', en: 'SENSEVOICE SMALL', category: '非实时普通识别', feature: 'record', model: 'sensevoice-small', abstract: '采集完成后整段转写。适合中英等多语言的快速识别，自动清理情感标签。', findings: ['使用 SenseVoice Small，GPU 优先、CPU 回退。', '支持麦克风、电脑音频和 WAV 文件。'] },
  ...
  { id: 'X-005', title: '实时识别 · SenseVoice', en: 'SENSEVOICE SMALL', category: '实时普通识别', feature: 'live', model: 'sensevoice-small', abstract: '分块重识别并实时更新文字。GPU 优先、CPU 回退，预览允许修订。', findings: ['使用 SenseVoice Small，同一份权重也支持非实时识别。', '语音结束后生成定稿。'] },
  ...
  { id: 'X-007', title: '目标说话人 · SenseVoice', en: 'TARGET SPEAKER', category: '目标说话人', feature: 'target', model: 'sensevoice-small', abstract: '仅转写与本机已注册声纹匹配的声音。FSMN-VAD + CAM++ + SenseVoice。', findings: ['先加载识别引擎和声纹组件；注册和删除声纹在 X-009。', '短语音可能跳过，不分离重叠声音；相似度不是概率。'] },
...
export const requiredModelIds: Record<string, readonly string[]> = {
  ...
  'X-002': ['fun-asr-nano'],
  'X-003': ['qwen3-asr'],
  'X-005': ['sensevoice-small'],
  'X-006': ['fun-asr-nano', 'fsmn-vad'],
  'X-007': ['sensevoice-small', 'fsmn-vad', 'cam-plus'],
...
export const speechArchives = entries.map(item => ({
  ...item, department: item.feature === 'models' ? 'MODEL REGISTRY' : item.model || 'FSMN-VAD + CAM++', date: '本地运行', lead: item.model === 'sample-x' ? 'AUTO' : item.feature === 'models' ? 'LOCAL' : item.feature === 'voice' ? 'CPU' : 'GPU',
  clearance: item.model === 'sample-x' ? 'LOCAL / AUTO' : item.feature === 'models' ? 'ALWAYS AVAILABLE' : item.feature === 'voice' ? 'LOCAL / CPU' : 'LOCAL / GPU',
