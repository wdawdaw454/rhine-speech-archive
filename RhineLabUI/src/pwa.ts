...
export function pwaSettingsMarkup() {
  return '<section class="pwa-settings"><h3>LOCAL / 本地语音服务</h3><p>语音工作台不保存离线页面或识别内容缓存。关闭本地后端后，转写不可用。</p></section>';
  /* Original archive PWA is not used by the local speech application.
...
export async function initPwa(notify: (message: string) => void) {
  // Never install an archive service worker over the local control surface.
  if (import.meta.env.BASE_URL === '/rhine/') return;
  if (isWallpaper) return;
  tell = notify;
